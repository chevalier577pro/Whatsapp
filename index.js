  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === 'connect') {
      ...
      try {
        ...
        const existing = sess.getSession(interaction.user.id);
        if (existing?.connected) {
          return interaction.editReply({ content: `✅ Tu es déjà connecté (${existing.phoneNumber || '?'}). Utilise \`/disconnect\` d'abord.` });
        }

        await interaction.editReply({ content: '⏳ Connexion à WhatsApp en cours... (jusqu a 30s)' });
        ...
        if (code) {
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              ...
              '2. ⚙️ Paramètres → **Appareils liés**\n' +
              '3. **Lier un appareil** → **Lier avec numéro de téléphone**\n' +
              '4. Entre le code ci-dessus\n\n' +
              '⏳ Le code expire dans **2 minutes**.'
            )
            .setColor(0x25d366)
            .setFooter({ text: 'Ne partage ce code avec personne !' });
          return interaction.editReply({ content: '', embeds: [embed] });
        } else {
          return interaction.editReply({ content: '🔄 Session en cours de restauration...' });
        }
      } catch (e) {
        console.error('Erreur /connect:', e.message);
        return interaction.editReply({ content: `❌ Erreur de connexion : ${e.message}\n\nVérifie que ton numéro est correct et réessaie.` });
      }
    }
