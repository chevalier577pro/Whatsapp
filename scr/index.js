...
// ── HTTP server pour afficher le QR ──────────────────────────
let lastQR = null;
http.createServer(async (req, res) => {
  if (lastQR) {
    ...
    res.writeHead(200, { 'Content-Type': 'text/html' });
    res.end(`<html><body style="background:#111;display:flex;align-items:center;justify-content:center;height:100vh">
      <p style="color:white;font-family:sans-serif;font-size:20px">✅ WhatsApp déjà connecté (ou en attente...)</p>
    </body></html>`);
  }
}).listen(process.env.PORT || 3000, () => {
  console.log(`🌐 QR server démarré sur port ${process.env.PORT || 3000}`);
});
...
waClient.on('qr', qr => {
  lastQR = qr;
  console.log('📱 QR Code prêt — ouvre le lien Railway public pour le scanner');
});
...
waClient.on('auth_failure', msg => {
  console.error('❌ Auth WhatsApp échouée:', msg);
});
...
waClient.on('message', async msg => {
  ...
  try {
    ...
    console.error('Erreur forward WA→DC:', e.message);
  }
});
